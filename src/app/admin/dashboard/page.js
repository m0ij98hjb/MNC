...
export default function DashboardPage() {
  ...
  const stats = [
    { key: 'total',        label: 'إجمالي الموردين' },
  ...
  return (
    <div className="p-6 lg:p-8 max-w-6xl mx-auto" dir="rtl">
      <div className="mb-8">
        ...
        <p className="text-sm text-white/40 mt-1">نظرة عامة على الموردين المسجلين</p>
      </div>

      {/* Stats grid */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-10">
        {stats.map(({ key, label }) => {
          const { icon: Icon, color, bg } = STAT_ICONS[key];
          return (
      ...
      <div className="bg-white/[0.02] border border-white/[0.07] rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.07]">
          <h2 className="text-sm font-semibold text-white">آخر الموردين المسجلين</h2>
        ...
        <div className="divide-y divide-white/[0.05]">
          {recent.length === 0 ? (
            <p className="text-center text-white/30 text-sm py-10">لا يوجد موردون بعد</p>
          ) : recent.map(s => (
            <Link
              key={s.id}
            ...
            >
              <div>
                ...
                <p className="text-xs text-white/30 mt-0.5">{s.city} · {s.phone}</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-white/20">
                  {s.createdAt?.seconds
                    ? new Date(s.createdAt.seconds * 1000).toLocaleDateString('ar-SA')
                    : ''}
                </span>
              </div>
            </Link>
