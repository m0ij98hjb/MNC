    ...
    <footer className="bg-[#eaeaea] py-8 border-t border-slate-300 w-full mt-auto">
      <div className="container mx-auto px-6">
        <div className={`flex flex-col md:flex-row justify-between items-center gap-6 ${lang === 'ar' ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
          {/* Logo */}
          <div className="flex items-center group ms-16 md:ms-20">
            <Image
              src="/asstes/logoFooter.png"
              alt="MNC Logo"
              width={200}
              height={90}
              className="logo-footer w-auto object-contain transition-all duration-500 group-hover:scale-105"
              priority
            />
          </div>

          {/* Copyright */}
          <p className={`text-slate-700 font-medium text-sm ml-20 md:ml-28 ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
